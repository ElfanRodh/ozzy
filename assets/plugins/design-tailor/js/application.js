"use strict";
angular.module("productApp", ["common.fabric", "common.fabric.utilities", "common.fabric.constants", "colorpicker.module", "ngSanitize", "ngMaterial", "ngScrollbar", "ngFileUpload"]).controller("ProductCtrl", [
  "$scope",
  "Fabric",
  "FabricConstants",
  "Keypress",
  ...
  function ($scope, Fabric, FabricConstants, Keypress, $http, $timeout, $mdDialog, $mdToast, Upload) {
    ...
    var SETTINGS_INIT_URL = "assets/plugins/design-tailor/inc/json/settings.json";
    ...
    $scope.clearAll = function () {
      var confirm = $mdDialog.confirm().title("").textContent("Are you sure you want to clear canvas? This will remove all design elements you have created.").ariaLabel("Confirm").ok("Ok").cancel("Cancel");
    ...
    $scope.removeSelectedObject = function () {
      var confirm = $mdDialog.confirm().title("").content("Are you sure you want to remove selected layer? this will remove the selected layer/design element.").ariaLabel("Confirm").ok("Ok").cancel("Cancel");
      $mdDialog.show(confirm).then(function () {
    ...
    $scope.deleteObject = function (object) {
      var confirm = $mdDialog.confirm().title("").content("Are you sure you want to remove selected object?").ariaLabel("Confirm").ok("Ok").cancel("Cancel");
      $mdDialog.show(confirm).then(function () {
    ...
    this.showNotification = function (message, scroll) {
      $mdToast.show($mdToast.simple().content(message).position($scope.getToastPosition()).hideDelay(3000));
      if (scroll) {
