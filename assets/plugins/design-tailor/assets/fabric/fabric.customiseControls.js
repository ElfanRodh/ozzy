...
(function (global) {
  ...
  if (global.fabric.version !== extCompat) {
    console.warn("this extension might not be fully compatible with your version " + "of fabric.js (" + global.fabric.version + ")." + "Consider using the latest compatible build of fabric.js" + extCompat);
  }
  ...
  fabric.util.object.extend(fabric.Object.prototype, {
    ...
    drawControls: function (ctx) {
      ...
      // top-left
      this._drawControl("tl", ctx, methodName, left - scaleOffset, top - scaleOffset, this.tlIcon);

      // top-right
      this._drawControl("tr", ctx, methodName, left + width - scaleOffset, top - scaleOffset, this.trIcon);

      // bottom-left
      this._drawControl("bl", ctx, methodName, left - scaleOffset, top + height - scaleOffset, this.blIcon);

      // bottom-right
      this._drawControl("br", ctx, methodName, left + width - scaleOffset, top + height - scaleOffset, this.brIcon);

      if (!this.get("lockUniScaling")) {
        // middle-top
        this._drawControl("mt", ctx, methodName, left + width / 2 - scaleOffset, top - scaleOffset, this.mtIcon);

        // middle-bottom
        this._drawControl("mb", ctx, methodName, left + width / 2 - scaleOffset, top + height - scaleOffset, this.mbIcon);

        // middle-right
        this._drawControl("mr", ctx, methodName, left + width - scaleOffset, top + height / 2 - scaleOffset, this.mrIcon);

        // middle-left
        this._drawControl("ml", ctx, methodName, left - scaleOffset, top + height / 2 - scaleOffset, this.mlIcon);
      }

      // middle-top-rotate
      if (this.hasRotatingPoint) {
        this._drawControl("mtr", ctx, methodName, left + width / 2 - scaleOffset, top - this.rotatingPointOffset - scaleOffset, this.mtrIcon);
      }
        ...
        if (cornerShape) {
          ...
          switch (cornerShape) {
            case "rect":
              ...
              if (icon !== undefined) {
                ctx[methodName](icon, left + cornerPadding / 2, top + cornerPadding / 2, size - cornerPadding, size - cornerPadding);
              }
              break;
            case "circle":
              ctx.beginPath();
